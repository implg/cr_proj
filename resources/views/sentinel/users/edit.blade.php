    ...
    <div class="row">
        <div class="col-md-10 col-md-offset-1">
            ...
            <div class="well">
                ...
                <div class="alert alert-warning" role="alert">Пользователи с ролью "Администратор" имеют доступ ко всем
                    предприятиям
                </div>

                <form method="POST" action="{{ route('update-branch-user', $user->hash) }}" accept-charset="UTF-8"
                      class="form-horizontal" role="form">

                    <div class="form-group">
                        <div class="col-sm-offset-2 col-sm-10 checkbox">
                            <label class="checkbox-inline">
                                <input class="full-access" type="checkbox" name="fullAccess"
                                       value="{{ $user->full_access }}" {{ $user->full_access == 1 ? 'checked' : '' }}>
                                Полный доступ
                            </label>
                        </div>
                    </div>
                            ...
                            @foreach(BranchUser::getListBranches() as $branch)
                                <label class="checkbox-inline">
                                    <input type="checkbox" name="branches[]"
                                           value="{{ $branch->id }}" {{ (BranchUser::inBranch($user->id, $branch->id) ? 'checked' : '') }}> {{ $branch->name }}
                                </label>
                            @endforeach
                        </div>
                    </div>
            ...
            <div class="well">
                ...
                <form method="POST" action="{{ $passwordFormAction }}" accept-charset="UTF-8" class="form-inline"
                    ...
                    <div class="form-group {{ $errors->has('newPassword') ? 'has-error' : '' }}">
                        ...
                        <input class="form-control" placeholder="Новый пароль" name="newPassword" value=""
                               id="newPassword" type="password">
                    </div>

                    <div class="form-group {{ $errors->has('newPassword_confirmation') ? 'has-error' : '' }}">
                        <label for="newPassword_confirmation" class="sr-only">Подтвердите пароль</label>
                        <input class="form-control" placeholder="Подтвердите пароль" name="newPassword_confirmation"
